# If-Signal-Then-Action Template

Use this template to turn adoption signals into operating decisions.

A KPI is only useful if it can change a decision, conversation or action.
...
## Example 1: Usage without workflow change
...
## Example 2: Training without activation
...
Pause:
Stop expansion until quality, risk or clarity improves.

Redesign:
Change the workflow, template, interface, handover or process.

Clarify:
Improve decision rights, ownership, criteria or communication.
...
Stop:
End a workflow, metric, pilot or practice that is not useful or safe.
